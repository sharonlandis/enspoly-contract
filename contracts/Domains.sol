...
//Change workspace compiler version (Remote) 
...
contract Domains is ERC721URIStorage {
  ...
  // constructor(string memory _tld) payable ERC721("Angel Name Service", "ANS") {
  //   tld = _tld;
  ...
  constructor(string memory _tld) ERC721 ("Angel Name Service", "ANS") payable {
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    //5 matic = 5 * 10**18, .5 matic = 5 * 10**17, .0005 matic = 5 * 10**14    
    if (len == 3) {
      return 5 * 10**14; 
    } else if (len == 4) {
      return 3 * 10**14; 
  ...
  function register(string calldata name) public payable {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Angel name service", "image": "data:image/svg+xml;base64,',
